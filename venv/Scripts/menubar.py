...
class MenuBar(Menu):
    ...
    def __init__(self, parent, nouveau, recup_info, option, timer, partie, pseudo_partie):
                ...
                if askokcancel('Attention', info):
                    with open('sauvegarde', 'a') as save:
                        ...
                        if retour[0]:
                            ...
                            showerror('Erreur', 'Votre partie à déjà été enregistré ' + str(retour[1]) + ' !')
        ...
        def score():
            ...
            if connecter() == False:
                ...
                showerror('Erreur de connexion',
                          "Vous n'êtes pas connecter, vous ne pouvez donc pas accéder aux scores.")
            else:
                classement = classer()
                ...
                for i in range(4):
                    for j in range(longueur):
                        ...
                        if i == 0:
                            ...
                            if j == 0:
                                texte = 'Niveau'
                            elif j > 0 and j <= expert:
                                texte = 'Expert'
                            elif j > expert and j <= expert + inter:
                                texte = "Intermédiaire"
                            elif j > inter + expert and j <= longueur:
                                texte = "Débutant"
        ...
        def options():
            ...
            s0 = Spinbox(mainFrame, state="readonly", values=('Débutant', 'Intermédiaire', 'Expert', 'Personnaliser'),
                         wrap=True, command=niveau)
            s0.pack()
            ...
            s2 = Spinbox(mainFrame, state=DISABLED, values=('Petite(9x9)', 'Moyenne(16x16)', 'Grande(30x16)'),
                         wrap=True, command=niveau)
            s2.pack(side=BOTTOM)
...
Demisn © 2016""", font=(police, 15)).pack()
