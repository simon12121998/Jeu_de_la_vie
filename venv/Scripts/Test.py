...
def damier():
    ...
    for i in range(c):
        for j in range(h):
            labels.append(
                Label(can, text=v, bd=1, justify=CENTER, relief=SUNKEN, font=("Helvetica", 9), image="", width=1,
                      height=1, padx=9, pady=5))
...
def chiffres(ref):
    ...
    if ref in top:
        for i in [ref - h, ref - (h - 1), ref + h, ref + (h + 1),
                  ref + 1]:  # On parcours les positions à côter de la bombe pour mettre si possible les bombes à proximités pour le HAUT
            if i >= 0 and i <= maxi:
        ...
        for i in [ref - h, ref - (h + 1), ref + h, ref + (h - 1),
                  ref - 1]:  # On parcours les positions à côter de la bombe pour mettre si possible les bombes à proximités pour le BAS
            if i >= 0 and i <= maxi:
                if labels[i].cget("text") == " ":
                    v = "1"
                elif labels[i].cget("text") in maxBomb:
        ...
        for i in [ref - h, ref - (h + 1), ref - (h - 1), ref + h, ref + (h - 1), ref + (h + 1), ref - 1,
                  ref + 1]:  # On parcours les positions à côter de la bombe pour mettre si possible les bombes à proximités pour les autres
            if i >= 0 and i <= maxi:
                if labels[i].cget("text") == " ":
                    v = "1"
                elif labels[i].cget("text") in maxBomb:
...
def verif(bool=False):
    ...
    for ref in range(c * h):
        ...
        if ref in top and b[ref].grid_info() == {} and labels[ref].cget("text") == v and labels[ref].cget(
                "image") == "":
            ...
            for i in [ref - h, ref - (h - 1), ref + h, ref + (h + 1), ref + 1]:
                ...
                if i >= 0 and i <= maxi and labels[i].cget("text") in maxBomb and labels[i].cget("image") == "":
                    b[i].grid_forget()
                elif i >= 0 and i <= maxi and labels[i].cget("text") == v and labels[i].cget(
                        "image") == "" and bool and i not in posForget:
                    b[i].grid_forget()
                    enlever_ranger(ref)
        ...
        elif ref in down and b[ref].grid_info() == {} and labels[ref].cget("text") == v and labels[ref].cget(
                "image") == "":
...
def nouveau(charger=False, bombe=[], pos=[], bouton=[], flag=[], name=""):
    ...
    for i in range(c):
        for j in range(h):
            labels.append(
                Label(can, text=v, bd=1, relief=SUNKEN, font=("Helvetica", 9), width=1, height=1, justify=CENTER,
                      padx=9, pady=5))
            labels[a].grid(column=i, row=j)
            b.append(Button(can, text="", image="", padx=8, pady=1))
            b[a].grid(column=i, row=j)
